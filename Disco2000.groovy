...
Date initialDate = new Date().parse("yyyy-M-d H:m:s", "2014-04-02 00:00:00")
...
Date destinyDate = args != 0 ? new Date().parse("yyyy-M-d", "2014-04-30") : new Date()
(initialDate..destinyDate.clearTime()).each { date ->
    ...
    fileLines.each { line ->
        try {
            ...
            String query = "(${slitted.get(0)}, ${slitted.get(1)}, '${date.clearTime().format("yyyy-MM-dd H:m:s")}', '${identifier}', 0),"
